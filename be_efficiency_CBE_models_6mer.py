...
def train_models(exp_nm, data, mut, ml_task):
  ...
  d = [list(s) for s in data['Local context']]
...
def gather_statistics(exp_nm, params):
  ...
  sixmer_nms = set(lib_design[lib_design['Char. Subset'] == '6mer']['Name (unique)'])
  data = data[data['Name'].isin(sixmer_nms)]
...
def gen_qsubs():
  ...
  for idx, row in treat_control_df.iterrows():
    ...
    out_fn = out_dir + '%s_%s_%s_model_stats.csv' % (treat_nm, 'C_TGA', 'regress_nonzero')
