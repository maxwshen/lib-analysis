...
# Default params
inp_dir = _config.OUT_PLACE + 'fulledits_general/'
...
def get_statistics(exp_nm):
  ...
  # Filter target sites with lower than 1% base editing fq
  ...
  num_bootstrap_rounds = 10000
  from scipy.stats.mstats import gmean
  gmeans, medians = [], []
  data = mdfs['Base edit to indel ratio']
  for bs_idx in range(num_bootstrap_rounds):
    bs_sample = np.random.choice(data, size = len(data), replace = True)
    gmeans.append(gmean(bs_sample))
    medians.append(np.median(bs_sample))

  stats = {
    'BS Geometric mean - 2.5th percentile': np.percentile(gmeans, 2.5),
    'BS Geometric mean - Mean': np.mean(gmeans),
    'BS Geometric mean - 97.5th percentile': np.percentile(gmeans, 97.5),
    'BS Median - 2.5th percentile': np.percentile(medians, 2.5),
    'BS Median - Mean': np.mean(medians),
    'BS Median - 97.5th percentile': np.percentile(medians, 97.5),
